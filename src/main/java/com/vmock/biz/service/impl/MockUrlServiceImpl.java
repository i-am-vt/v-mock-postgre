...
public class MockUrlServiceImpl extends ServiceImpl<MockUrlMapper, MockUrl> implements IMockUrlService {
    ...
    private static final String SQL_REGEXP = "*, regexp_matches(logic, %s)";
    ...
    public MockUrl getUrlByRegexp(String requestUrlLogic) {
        if (StrUtil.isNotBlank(requestUrlLogic)) {
            ...
            // 查询
            QueryWrapper<MockUrl> queryWrapper = Wrappers.query();
            return this.getOne(queryWrapper.select(String.format(SQL_REGEXP, regexp)));
        }
        return null;
    }
